...
- Verificado: **25 e2e** (6 novos cobrindo a revisão: grifo pintado, estojo sem cor, borracha apagando, limpar com confirmação, clique fora não guardando a borracha, e a ausência de estojo sem grifo) e 915 unitários. Conferido ponta a ponta contra o stack local: grifar → finalizar → revisão.
...
- **O teto de 3 tentativas vitalícias deixou de ser um balde único e virou dois.** `nacional` (treino PRONTO, via `iniciar_tentativa`) = 2, `montado` (via `gerar_simulado_personalizado`) = 1. `limite_tentativas_gratuitas()` continua valendo 3 e continua sendo o número que a landing anuncia — mas agora é **derivada** da soma dos dois, para o total nunca divergir dos baldes.
- **Por que dois baldes e não um saldo livre de 3:** com balde único o aluno gastaria tudo em prova pronta e nunca provaria o montador, que é justamente o produto do Avançado. Separar garante que toda conta gratuita experimente as duas coisas.
- **A escada de planos inverte de propósito e isso está documentado.** Montar simulado passa a ser o único recurso que o gratuito tem e o **Essencial (pago) não tem** — o Essencial continua barrado com P0015, de caso pensado. O que separa os dois planos é o teto de tentativas, não o catálogo: o grátis monta uma vez na vida, o Essencial treina sem limite no conteúdo que ele vende. Registrado em `docs/business-rules.md` com um aviso para ninguém "consertar" isso depois.
- **O simulado montado do gratuito sorteia do acervo inteiro** — nacional, processual e laboratório, objetivas ou discursivas. É a amostra grátis do Avançado, limitada a uma geração. Custo assumido junto: uma tentativa discursiva por conta gratuita consome correção da Aurora, e `corrigir-resposta-aberta` não tem gate de tier.
- **Cap global por cima dos baldes, senão haveria 4ª tentativa de brinde.** Cada saldo é `greatest(0, least(limite_do_balde − usadas_do_balde, 3 − usadas_no_total))`. Sem o segundo termo, a conta legada que já queimou 3 nacionais na regra antiga ganharia um montado novo. Com ele, quem bateu no teto segue bloqueado nos dois caminhos.
- **Como os baldes são medidos, sem coluna nova nem backfill:** `prova.origem = 'personalizado'` marca a prova sintética que só `gerar_simulado_personalizado` cria. Os conjuntos são disjuntos por construção — prova personalizada não tem linha em `prova_questao`, então `iniciar_tentativa` nunca a aceita (P0004).
- **`gerar_simulado_personalizado` trocou o gate binário pelo gate por nível:** saiu o `tem_assinatura_ativa()` (P0009), entrou `nivel_acesso()` — essencial cai em P0015, gratuito sem crédito cai em P0016, gratuito com crédito passa.
- **`listar_temas_com_contagem` virou `SECURITY DEFINER`, e sem isso a feature nasceria quebrada:** o RLS de `questao` exige `tem_assinatura_ativa()`, então a tela de montar abriria com 0 questões em todo tema para o gratuito e o botão desabilitado. A função devolve só contagem agregada por tema — verificado em teste que o gratuito lê 13 temas com contagem e continua lendo **0 linhas** de `questao` direto.
- **`get_status_acesso` passou a devolver os dois baldes** (`nacional_*`, `montado_*`) no mesmo payload, sem segunda RPC. O total sozinho não serve para a UI: com `restantes = 1` o aluno pode ter 1 treino pronto OU 1 montado, que são caminhos diferentes. Por isso a tela de detalhe da prova agora decide o botão "Iniciar" pelo balde nacional, não pelo total — antes o label prometeria "1 grátis" e o servidor recusaria com P0016.
- **Guard novo em `/dashboard/simulados/montar`:** `montarSimuladoGuard` barra **apenas** o essencial. O gratuito sem crédito entra na tela e lê o aviso ali — barrar a rota esconderia o recurso de quem está no momento exato de decidir assinar. O botão vira "Assinar para montar de novo" e leva a `/planos?origem=limite-tentativas`.
- **Impressão não foi junto:** segue benefício de assinante (`gerar_simulado_impressao` recusa o gratuito com P0009, `/imprimir` tem o `nivelPagoGuard`), então o botão "Apenas imprimir" simplesmente não é renderizado para o gratuito, em vez de existir só para dar erro.
- **Copy atualizada onde o número aparecia como "3 simulados" genérico:** landing (card do plano grátis, parágrafo da seção de planos, FAQ de laboratório), `/planos` (contextos `limite-tentativas` e `simulado-personalizado`, listas de benefícios do Essencial e do Avançado), paywall in-app, banner de tentativas (agora quebra o saldo: "Ainda dá para fazer 2 treinos nacionais e 1 simulado montado por você") e a tela Minha assinatura. O "3 simulados grátis" do hero segue verdadeiro — o total não mudou.
- Verificado: migration aplicada no banco local e exercitada em transação com `ROLLBACK` como usuário `authenticated` real — fluxo completo do gratuito (2 nacionais → 3ª recusada com P0016 → 1 montado → 2º recusado com P0016), conta legada com 3 nacionais sem ganhar o montado, essencial recusado com P0015, avançado montando 2× sem limite, `anon` recebendo 42501 nas funções novas e `authenticated` recebendo 42501 no helper interno `tentativas_gratuitas_usadas`. `tsc` limpo, build de produção OK, **837 unitários verdes** (+4) e **25/25 e2e** em `tier-essencial.spec.ts` (+5). A suíte `mocked` completa tem 56 falhas pré-existentes, idênticas antes e depois da mudança (conferido com `git stash`).
- **Aplicado em produção** em 08/09 por `db push --linked`, com `--dry-run` antes e `seeds` vazio nas duas vezes. Conferido no banco de prod: limites 3/2/1, `iniciar_tentativa` debitando do balde nacional, `gerar_simulado_personalizado` sem `tem_assinatura_ativa`, `listar_temas_com_contagem` como DEFINER, `anon` sem EXECUTE em nenhuma função nova e `authenticated` sem EXECUTE no helper interno.
- **Follow-up imediato (`20260908130000`):** o `get_advisors` acusou `function_search_path_mutable` nas três funções de limite — duas nascidas nesta feature e a `limite_tentativas_gratuitas()`, que já vinha assim desde 20260801115817. Migration separada fixando `search_path` nas três; advisor voltou a zero para esse lint. O resto do relatório é a linha de base conhecida do projeto (86 SECURITY DEFINER para `authenticated`, o unsubscribe público intencional, 3 tabelas com RLS sem policy).
- **O push esbarrou numa divergência repo↔prod de nomes de migration**, resolvida no commit anterior: três arquivos aplicados direto em produção tinham ficado com um timestamp diferente do registrado em `supabase_migrations.schema_migrations`, e o `db push` recusava rodar inteiro. Renomeados para a version real (`20260904170000 → 20260904170124`, `20260905120000 → 20260905235528`, `20260905130000 → 20260905235701`), sem tocar no SQL. É o mesmo ajuste que `b37cea3` já tinha feito no `admin_ranking_ia_usuarios`; sem ele, uma vez destravado, o push trataria os três como pendentes e tentaria reaplicá-los em prod — inclusive as duas migrations de DADOS do Riquelme.
- **Dois achados anotados de passagem em `despesa`**, encontrados ao conferir o schema de prod contra o repo, ambos fora do escopo desta feature e pendentes de decisão: (1) a tabela tem os grants default para `anon` — hoje inofensivo, porque a RLS está ligada e as quatro policies exigem `is_admin()`, mas o revoke não está aplicado; (2) o trigger é `BEFORE INSERT` apenas, embora `despesa_touch()` tenha um ramo de UPDATE — efeito: `atualizado_em` nunca muda e as proteções de `criado_em`/`criado_por` contra edição não rodam.
- `docs/business-rules.md` atualizado.
...
- **O ponto de partida era um vazio, não um dado bruto.** `auth.sessions` já traz `ip` e `user_agent` (as 90 sessões vivas em produção têm IP), mas a linha morre no logout/expiração, e `auth.audit_log_entries` está zerada — a plataforma purga. Sem tabela própria não havia histórico com o que comparar. `public.acesso_log` é essa tabela.
- **Duas fontes alimentam o log.** Triggers em `auth.sessions` (`AFTER INSERT`, e `AFTER UPDATE` com `WHEN NEW.ip IS DISTINCT FROM OLD.ip`) capturam todo login novo e toda troca de IP dentro de uma sessão — server-side, sem depender do frontend, valendo para qualquer cliente. E o RPC `registrar_acesso(p_device_id)`, heartbeat que o layout do dashboard dispara ao montar e a cada 30 min.
- **Só login não resolveria o problema pedido.** Duas pessoas que entram uma vez e ficam logadas por semanas geram dois logins e nenhum sinal de uso paralelo. O heartbeat é o que produz **janelas de presença** e, com elas, a detecção de acesso simultâneo em redes diferentes — que é o indício que separa compartilhamento de deslocamento.
- **De onde sai o IP, verificado empiricamente em produção** (função temporária, chamada por curl, removida em seguida): há Cloudflare na frente do Supabase, e o `request.headers` do PostgREST chega com `cf-connecting-ip`, `x-forwarded-for` **e** `cf-ipcountry`. O RPC lê `cf-connecting-ip` primeiro — a borda reescreve esse header, então o cliente não o forja —, cai para o primeiro elemento de `x-forwarded-for` e aproveita o país de graça, sem API de geolocalização.
- **Cada linha é uma janela, não um evento.** `registrar_acesso_evento()` consolida eventos do mesmo usuário/IP/dispositivo dentro de 30 min em `ultimo_em`/`eventos`. Efeito colateral desejado: o heartbeat se junta à linha criada pelo login e preenche nela o `device_id` que o trigger não tinha como saber. Volume fica em ~1–3 linhas por usuário-dia.
- **`device_id` é um UUID em `localStorage`**, gerado pelo `AcessoService`. Serve para distinguir dois aparelhos atrás do mesmo IP e para seguir o mesmo aparelho trocando de rede. O usuário pode apagar — é indício, nunca prova, e está comentado como tal no código.
- **Trigger em schema `auth` é risco de login, e está tratado como tal.** O corpo inteiro de `trg_acesso_sessao()` roda em bloco `EXCEPTION WHEN OTHERS THEN NULL`: ele executa dentro da transação do GoTrue, e monitoramento quebrado não pode impedir alguém de entrar. Segue o precedente que já existia no projeto (`trg_sync_ultimo_login` em `auth.users`).
- **Impersonação de admin seria falso positivo garantido:** ela cria uma sessão real do usuário-alvo a partir do navegador do admin. O trigger cruza `admin_impersonation_log` (5 min anteriores) e marca `impersonado`; toda análise filtra essas linhas, e o heartbeat não roda sob impersonação (o `effect` do dashboard já distinguia esse caso).
- **Ruído tratado nas próprias consultas.** IP agrupado por `/24` (v4) / `/48` (v6) em `rede_do_ip()`, para IP dinâmico da mesma casa não contar como rede nova; pares de mesmo `device_id` ignorados na detecção de simultaneidade, para wi-fi → 4G no mesmo aparelho não virar acusação; tolerância de 10 min ao cruzar janelas.
- **O score na tela é aritmética auditável, não classificador**: redes, dispositivos, países e sobreposições, com peso explícito, teto em 100 e faixas alto/médio/baixo. CGNAT de operadora e wi-fi de faculdade produzem falso positivo inevitável — as duas telas dizem isso em texto e mostram a evidência bruta, porque a decisão é humana.
- **`/admin/acessos`** (menu Gestão): ranking de contas com filtro de 7/30/90 dias, drill-down por usuário (totais, pares de uso simultâneo, agregado por IP, histórico) e aba "Redes com várias contas" — o ângulo inverso, muitas contas num lugar só.
- **Acesso fechado:** RLS admin-only para SELECT, **nenhuma** policy de escrita (a gravação é exclusiva das funções `SECURITY DEFINER`), `REVOKE` de `anon`/`PUBLIC` nas RPCs e `REVOKE` também de `authenticated` nas internas. Verificado por curl: aluno chamando RPC admin recebe `permission_denied`, aluno lendo a tabela recebe `[]`, aluno tentando `INSERT` recebe 42501, anon nas duas funções internas recebe 42501.
- **Teto anti-abuso no heartbeat:** um cliente malicioso poderia inflar a tabela variando `p_device_id` a cada chamada, então `registrar_acesso` para de criar linhas novas acima de 20 por hora por usuário.
- **Retenção de 180 dias** via `purgar_acesso_log()` agendada em `pg_cron` (04:17 diário). A coleta de IP já estava declarada na política de privacidade ("dados técnicos… para segurança e diagnóstico").
- **Achado que fica pendente, fora do escopo:** os **termos de uso não proíbem** compartilhar conta — proíbem copiar conteúdo, criar múltiplas contas e burlar autenticação, nada sobre conta pessoal e intransferível. Sem essa cláusula, não há base contratual para agir sobre o que o monitoramento mostrar. Registrado em `docs/business-rules.md`.
- Verificado: migration aplicada no Supabase local, login real via GoTrue gravando a linha pelo trigger, heartbeat consolidando e criando linha por `device_id` distinto, as três RPCs conferidas contra dados sintéticos de sobreposição, `tsc` limpo, build de produção OK, **832 unitários verdes** (6 novos em `acesso.service.spec.ts`) e a tela navegada no browser contra o banco local (ranking, drill-down e aba de redes).
- **Aplicado em produção** em 04/09 por `db push --linked --include-all` (caminho documentado em `supabase/CLAUDE.md`, com `--dry-run` antes e `seeds` vazio). Conferido no banco de prod: 90 linhas de carga inicial vindas de `auth.sessions`, 2 triggers em `auth.sessions`, RLS ligada com 1 policy, cron `purgar-acesso-log` agendado. Guards reconferidos por curl contra prod com a anon key: `registrar_acesso`, `admin_get_acessos_resumo`, SELECT e INSERT na tabela todos 42501. `get_advisors` não trouxe nenhum alerta novo — só o `authenticated_security_definer_function_executable` esperado em `registrar_acesso`, que é o padrão de outras 80 funções do projeto.
- **O push destravou duas divergências repo↔prod que estavam escondidas:** (1) prod tinha a migration `20260903125850_cupom_iza15` aplicada direto no banco em 03/09, sem arquivo local — o `db push` recusava rodar por causa dela; o SQL foi recuperado do `supabase_migrations.schema_migrations` de prod e reconstruído no repo, mesmo tratamento que MA20 e MARIBRASIL já tinham recebido. (2) `20260902120000_notificacoes_expiram_apos_leitura` estava commitada desde 02/09 mas **nunca havia sido aplicada** — coluna `lida_em`, função, trigger e índice não existiam em produção, enquanto o frontend que lê `lida_em` já estava no ar. Era exatamente o cenário que `supabase/CLAUDE.md` alerta ("banco antes do frontend"), com efeito degradado e silencioso: a expiração de 7 dias nunca acontecia. Foi aplicada junto, com aprovação explícita.
- `docs/architecture.md` (ADR-036) e `docs/business-rules.md` atualizados.
...
- **O cupom não vive em `pagamento`.** O snapshot fica em `pagamento_intencao.cupom_id/desconto_centavos` (checkout embutido) e o pagamento aponta para a intenção por `pagamento.intencao_id`. A migration `20260831182947` recria `admin_listar_pagamentos` com dois `LEFT JOIN` nesse caminho (`pagamento_intencao` → `cupom`) e devolve `cupom_codigo` + `desconto_centavos`.
...
- **Aplicada em produção** (projeto `gakvktwtdunljojghpff`) via MCP, com autorização explícita do Guilherme — migrations não saem por CI. Prod não tinha drift: a função lá era idêntica à do repo antes da troca. Conferido depois de aplicar: 45 pagamentos no total, 21 com intenção, 11 com cupom (CALOURO20 ×6, MA20 ×3, YAS20 ×2) e desconto > 0; o nome do arquivo segue a versão gravada no ledger.
