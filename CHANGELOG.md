...
**Formatação da explicação das questões: correção dos dados da TPI 2025.1 e lacuna no pipeline**

A TPI 2025.1 (120 questões, ainda não publicada) exibia a explicação de cada questão como um bloco só de prosa — o comentário de cada alternativa colado no da seguinte, sem parágrafo algum — enquanto o enunciado já tinha esse problema resolvido por `formatarEnunciado`.

- **Causa raiz**: `colapsarParagrafos()` em `scripts/importar-prova-scan/gerar.mjs` remove linhas em branco de todo campo de texto livre na geração do markdown (mesmo tratamento para `enunciado`, `enunciado_apoio` e `explicacao`). O enunciado tem `formatarEnunciado` reconstruindo os parágrafos em tempo de exibição; a explicação não tinha equivalente — por isso toda devolutiva comentada nascia ilegível, não só a da TPI.
- **Dados**: reformatadas as 120 explicações da TPI (107 precisaram de ajuste, 13 já estavam ok), inserindo apenas quebras de parágrafo nos pontos estruturais (rótulos "Distratores:"/"RESPOSTA CORRETA", comentário por alternativa, enumerações romanas I–IV) — conferido mecanicamente que, removendo todo espaço em branco, o texto novo é idêntico ao original em cada uma das 120 questões (nenhuma palavra alterada). Texto original preservado em `explicacao_original` (coluna já existia na tabela `questao`) antes da sobrescrita.
- **Bugs de conteúdo achados e corrigidos nesse processo** (não são formatação): questão 45 com "INCORRETAA" grudado (espaço faltando); questão 56 com cabeçalho "Área/Subárea/Tema" vazado dentro da explicação (removido); questão 59 com "[ara" no lugar de "para" (típico erro de OCR, colchete por P); questão 96 com uma frase repetida duas vezes seguidas (deduplicada); questão 118 com "crotálico:O veneno" sem espaço após o rótulo; e questão 75, mais grave — a explicação se referia às alternativas por ordinal ("a primeira", "a quarta alternativa está correta") mas a ordem citada não batia com a letra real marcada como gabarito no banco (a alternativa correta é a **C**, citada como "quarta"); os ordinais foram trocados pela letra real de cada alternativa (`A`–`E`), mapeados por conteúdo contra `alternativa.texto`.
- **Pipeline**: `parseQuestaoBloco()` em `admin-importar.component.ts` descartava linhas em branco dentro da seção `EXPLICACAO` (`if (secao === 'explicacao' && t)`) enquanto `ENUNCIADO` as preservava — agora ambas tratam blank lines da mesma forma, então uma devolutiva colada já com parágrafos não perde essa estrutura na importação.
- **Novo `formatarExplicacao`** (`shared/utils/formatar-explicacao.ts` + `FormatarExplicacaoPipe`), aplicado em `questao-explicacao.component` (bloco de prosa e `intro` da explicação estruturada): insere parágrafo antes de rótulos de seção (Distratores/Resposta(s) Correta(s)/Incorreta(s)/Justificativa/Gabarito/Referências), antes de "A alternativa X está correta/incorreta", antes do prefixo "incorreta -"/"correta -" e em enumerações romanas — reaproveitando `detectarEnumeracaoRomana`/`separarItensRomanos`, agora exportados de `formatar-enunciado.ts`. Cobertura parcial por natureza (documentado no próprio arquivo): só ~30–45% das devolutivas têm rótulo ou padrão reconhecível; o resto é prosa livre sem marcador e continua exigindo revisão editorial — o mesmo limite que já existia para o crivo 3 do pipeline de importação. 6 testes novos em `formatar-explicacao.spec.ts`, suíte inteira (754 testes) verde.
