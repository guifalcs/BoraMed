...
* Na listagem de treinos nacionais, a prova que o aluno já concluiu recebe um selo discreto (ícone de check, texto no hover). "Concluída" = existe tentativa `finalizada` do próprio aluno, excluindo modo `visualizar` — ver gabarito não conta como ter feito a prova. Tentativa `em_andamento`/`pausada` não marca o selo.
...
* **Troca de formato durante a prova:** onde existe gêmea, o aluno pode trocar a questão pela variante do outro formato sem sair da execução (`trocar_formato_questao_tentativa`). Regras: só em prova não-nacional (é onde existem gêmeas), só com a questão **intocada** (não respondida e não enviada — depois disso a discursiva já consumiu correção de IA e a fechada já pode ter revelado o gabarito em modo estudo), nunca para uma gêmea que já esteja na mesma tentativa, e nunca para gêmea arquivada/anulada pelo admin. A troca reescreve `tentativa_resposta.questao_id` da linha: ordem na prova, marcação de revisão e anulação do aluno seguem a questão; o rascunho de texto é descartado (o enunciado muda). Resultado, revisão, nota, distribuição por tema e stats de questão derivam de `tentativa_resposta`, então acompanham sozinhos. Modo `visualizar` não oferece troca.
...
* **Eliminar alternativas durante a resolução:** o aluno pode riscar as alternativas que já descartou, só para organizar o raciocínio. Três caminhos para a mesma ação: botão `X` no hover da alternativa (mouse), `Shift` + `A`–`E` / `1`–`5` (teclado) e segurar a alternativa por 500ms (toque). É apoio visual e nada mais — não conta como resposta, não vai para o banco, não aparece em resultado, revisão, nota ou estatística de questão. O estado fica no `sessionStorage` do navegador (`bm_eliminadas_<tentativa_id>`), sobrevive a um F5 no meio da prova e é apagado ao finalizar. Invariante: a alternativa marcada não pode ser riscada e a riscada não pode ser marcada — para escolher uma alternativa riscada é preciso restaurá-la antes. Riscar não fica disponível em modo `visualizar`, em questão discursiva, nem depois que o gabarito aparece na tela (modo estudo já respondido) — nesses casos as riscas somem.
* **Grifar o texto da questão (marca-texto):** durante a resolução o aluno pode grifar trechos do enunciado, do texto de apoio e das alternativas, em **qualquer cor do espectro**, como faria numa prova impressa. A paleta traz quatro atalhos (amarelo, verde, azul, rosa) e um botão que abre uma **grade com 44 cores** (10 matizes × 4 luminosidades + 4 neutros) cobrindo o círculo de cores; as cores escolhidas na grade viram atalho (as 4 mais recentes) e ficam salvas em `bm_grifo_paleta`, que é preferência do aluno e sobrevive à tentativa. **A cor do texto grifado é calculada por contraste** (luminância WCAG): pastel fica com texto escuro, cor escura inverte para branco — sem isso, escolher um roxo forte apagaria o enunciado que se quis destacar. O estojo é um dock flutuante escondido na borda direita, empilhado sobre a aba do widget de suporte — guardado, aparece só a ponta e ele desliza para fora no hover; o modo é explícito (botão ou atalho `G`, que vale também na revisão) porque no toque a seleção de texto disputa o gesto com o long press que risca alternativa — com o marca-texto na mão, quem manda é a seleção. **Atalhos `G` e `Shift + G`:** com texto selecionado, `G` grifa o trecho na cor em uso e `Shift + G` apaga — os dois na hora, sem precisar pegar a ferramenta antes e **sem trocar a que está na mão** (é uma passada, não uma troca de ferramenta). Sem seleção, cada um entrega a sua ferramenta: `G` o marca-texto (na última cor usada), `Shift + G` a borracha; apertado com a própria ferramenta já na mão, guarda. **Com a borracha na mão, `G` devolve a caneta em vez de guardar tudo** — quem pede o marca-texto quer grifar, e ter de clicar numa cor depois anularia o atalho. O botão principal do estojo segue a mesma regra. `B` e `E` não servem de atalho porque respondem alternativa. Valem nas duas telas e são ignorados dentro de campo de texto, com diálogo aberto e **fora de uma tentativa** — o card de questão também renderiza no admin e no gabarito público (`/visualizar`), onde não há grifo para salvar. Com o modo ativo, **o cursor vira a própria ferramenta em cima do texto que aceita pintura** — caneta com a ponta na cor carregada (qualquer que seja, com o contorno clareando em cor escura), bloco para a borracha —, e é o aviso de onde o marca-texto pega; selecionar um trecho grifa na cor escolhida; a borracha apaga o que estiver sob a seleção e "limpar" zera a questão atual. Regra do marca-texto de verdade: a última passada manda — grifar por cima com outra cor substitui a anterior no trecho coberto. É apoio de leitura e nada mais — não conta como resposta, não vai para o banco, não aparece em resultado, revisão, nota ou estatística de questão. O estado fica no `localStorage` do navegador (`bm_grifos_<tentativa_id>`, formato v2 com a cor em hex — payload v1, com as quatro cores por nome, ainda é lido e traduzido; teto de 200 trechos por questão), sobrevive a F5 e a pausar/retomar a tentativa, **sobrevive à finalização** e chaves de tentativas com mais de 30 dias são varridas sozinhas. Com o marca-texto na mão, um clique limpo na alternativa segue respondendo; só o clique que termina uma seleção é ignorado. **Clique em lugar nenhum guarda o marca-texto** — seguram a caneta na mão o próprio estojo, qualquer controle (alternativa, navegação, finalizar) e o texto grifável; o resto é fundo de tela. Um arrastar que começa no texto e termina fora não conta: o que decide é onde o toque começou. Âncora do grifo: intervalo de caracteres dentro do bloco (enunciado/apoio/alternativa), não nó do DOM — por isso ele reancora sozinho quando o Markdown re-renderiza. A pintura usa a CSS Custom Highlight API; navegador sem suporte mostra a prova normal, sem a pintura.
* **Grifos na revisão pós-prova:** o que o aluno grifou durante o simulado continua à vista na revisão (`/revisao`), por cima da prova já corrigida. Ali o estojo **só oferece borracha e limpar** — grifar na revisão misturaria o que foi marcado sob o relógio com o que foi marcado depois, já lendo o gabarito. O estojo só aparece se existir algum grifo salvo. Como a revisão lista a prova inteira numa página só, ali o "limpar" vale para **todos os grifos da revisão** e pede confirmação — "esta questão" seria ação destrutiva com alvo invisível, já que o estojo flutua sobre uma lista com todas elas. Na execução, onde há uma questão na tela, o alvo continua sendo ela, sem confirmação. O clique fora guarda a ferramenta também na revisão, pela mesma regra da execução. E quando não sobra grifo nenhum ela é guardada sozinha: o estojo some junto, e a borracha na mão deixaria o cursor preso no texto sem botão para desligar. A revisão de prova sem tentativa (`/visualizar`, gabarito público) não tem grifo nenhum.
...
* **Lista agrupada por tema principal** (accordion colapsado por padrão, evita virar parede de cards com volume alto): `get_caderno_erros` devolve `tema_principal` — cascata `questao_tema.principal` → primeiro tema alfabético → `disciplina.sigla` → `tipo_questao` — além do array `temas` completo. A cascata existe porque 91% das questões nacionais em prod (checado 18/09) não têm tema nenhum e 47% nem disciplina; sem ela, quase todo erro nacional cairia junto num "Sem tema" gigante. Buscando por texto, a lista some do agrupamento (fica plana).
* **Gate de tier essencial usa `tipo_questao = 'nacional'`, NUNCA `formato_prova`** — checado em prod: `formato_prova` está NULL em 100% das questões ativas (nenhum fluxo de import o preenche), então gatear por ele deixaria o Caderno de Erros vazio pra todo aluno Essencial.
* RPCs: `get_caderno_erros` (lista com filtros de tema/disciplina/tipo/status, busca por texto no enunciado via `ilike` e paginação server-side — `p_pagina`/`p_por_pagina`, máx. 100/página, `total_count` via window function), `get_caderno_erros_resumo` (KPIs: pendentes, por tipo, tema mais fraco — mesmo corte ≥3 de `get_desempenho_por_tema` —, dominadas nos últimos 7 dias), `get_questao_para_refazer` (questão para responder avulso), `responder_questao_avulsa` (corrige server-side só formatos determinísticos — `resposta_aberta_curta` não é suportado — e faz upsert em `caderno_erro_status`, sem gravar `tentativa`/`tentativa_resposta`), `marcar_questao_dominada` (upsert manual de status).
...
* `gerar_simulado_personalizado` ganhou o parâmetro opcional `p_apenas_erros` (default `false`, sem quebrar chamadas existentes): quando `true`, monta o simulado só com o pool de questões erradas pendentes do aluno; exige assinatura ativa mesmo no plano gratuito (o crédito único de "montar simulado" do free tier não cobre esse modo). O nome da prova fica "Simulado dos meus erros - N questões" e aparece assim no Histórico (removida a troca hardcoded por "Simulado Personalizado" que existia em `historico.service.ts` para toda prova `origem='personalizado'`).
* `sincronizar_caderno_erro_status_pos_tentativa(p_tentativa_id)`: chamada fire-and-forget pelo client logo após `finalizar_tentativa` (`TentativaService.finalizar`), para QUALQUER tentativa. Se uma questão que já era erro rastreado for respondida certa nesta tentativa real, vira `dominada`; se errar de novo, volta a `pendente`. No-op para questão que nunca foi erro (não polui a tabela). `get_caderno_erros` filtra `ur.nota < 70 OR ces.questao_id IS NOT NULL` (não só `ur.nota < 70`) por causa disso — senão uma questão dominada por essa via (nota real já ≥ 70) sumiria de "Mostrar dominadas" mesmo com `status='dominada'`.
