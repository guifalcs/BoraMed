...
* **Grifar o texto da questão (marca-texto):** durante a resolução o aluno pode grifar trechos do enunciado, do texto de apoio e das alternativas, em quatro cores (amarelo, verde, azul, rosa), como faria numa prova impressa. O estojo é um dock flutuante no rodapé; o modo é explícito (botão ou atalho `G`) porque no toque a seleção de texto disputa o gesto com o long press que risca alternativa — com o marca-texto na mão, quem manda é a seleção. Com o modo ativo, selecionar um trecho grifa na cor escolhida; a borracha apaga o que estiver sob a seleção e "limpar" zera a questão atual. Regra do marca-texto de verdade: a última passada manda — grifar por cima com outra cor substitui a anterior no trecho coberto. É apoio de leitura e nada mais — não conta como resposta, não vai para o banco, não aparece em resultado, revisão, nota ou estatística de questão. O estado fica no `localStorage` do navegador (`bm_grifos_<tentativa_id>`, teto de 200 trechos por questão), sobrevive a F5 e a pausar/retomar a tentativa, é apagado ao finalizar, e chaves de tentativas com mais de 30 dias são varridas sozinhas. Com o marca-texto na mão, um clique limpo na alternativa segue respondendo; só o clique que termina uma seleção é ignorado. Âncora do grifo: intervalo de caracteres dentro do bloco (enunciado/apoio/alternativa), não nó do DOM — por isso ele reancora sozinho quando o Markdown re-renderiza. A pintura usa a CSS Custom Highlight API; navegador sem suporte mostra a prova normal, sem a pintura.
