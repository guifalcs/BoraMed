...
as $function$
declare v_limite integer; v_user_id uuid;
begin
  ...
  return (
    ...
    top_n as (select * from ranking where posicao <= v_limite),
    eu_fora as (
      select * from ranking
      where v_user_id is not null and user_id = v_user_id and posicao > v_limite
    ),
    combinado as (select * from top_n union all select * from eu_fora)
    select coalesce(jsonb_agg(jsonb_build_object(
      'user_id',user_id,'nome_display',nome_display,'nivel',nivel,
      'xp_total',xp_total,'xp_semana_atual',xp_semana_atual,'posicao',posicao,'is_me',is_me
