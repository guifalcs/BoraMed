-- ============================================================================
-- Questões abertas — Fase 4: simulado personalizado com discursivas.
--
-- `gerar_simulado_personalizado` ganha `p_formato_questao`:
--   'fechadas'    (default) → só múltipla escolha / V-F (comportamento atual;
--                             também blinda simulados existentes de receberem
--                             discursivas recém-cadastradas)
--   'discursivas'           → só resposta_aberta_curta
--   'misto'                 → sem filtro de formato
--
-- O payload passa a emitir o gabarito aberto (resposta_modelo/pontos_chave/
-- criterios_correcao) mascarado em modo simulado — mesmo padrão de
-- iniciar/retomar_tentativa (20260707130000).
--
-- ⚠️ AVISO ANTI-REGRESSÃO DE GRANTS: não regenerar via `db pull`/`db diff`.
-- ============================================================================

-- Assinatura muda (parâmetro novo): remove a versão antiga para não deixar
-- overload ambíguo no PostgREST.
drop function if exists public.gerar_simulado_personalizado(uuid[], integer, text, text, text);
