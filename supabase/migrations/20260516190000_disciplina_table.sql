-- ============================================================
-- Tabela disciplina — matéria do período
-- Fluxo: disciplina (matéria + período) → tema → questão
-- ============================================================
...
BEGIN
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'temas', (
        ...
        LEFT JOIN disciplina dt ON dt.id = t.disciplina_id
  ...
  INTO v_questoes
  FROM questao q
  LEFT JOIN disciplina dq ON dq.id = q.disciplina_id
  WHERE q.prova_id = p_prova_id AND q.status = 'ativa';
...
BEGIN
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', q.id, 'prova_id', q.prova_id, 'ordem_na_prova', q.ordem_na_prova,
      'enunciado_apoio', q.enunciado_apoio, 'enunciado', q.enunciado,
      'imagem_url', q.imagem_url, 'imagem_legenda', q.imagem_legenda,
      'formato', q.formato, 'explicacao', q.explicacao, 'dificuldade', q.dificuldade,
      'disciplina', dq.sigla, 'periodo', dq.periodo::int, 'status', q.status,
      'criado_em', q.criado_em, 'atualizado_em', q.atualizado_em,
      'alternativas', (
        SELECT jsonb_agg(jsonb_build_object(
          'id', a.id, 'questao_id', a.questao_id, 'letra', a.letra, 'texto', a.texto,
          'correta', a.correta, 'ordem', a.ordem, 'imagem_url', a.imagem_url
        ) ORDER BY a.ordem)
        FROM alternativa a WHERE a.questao_id = q.id
      ),
      'temas', (
        SELECT jsonb_agg(jsonb_build_object(
          'id', t.id, 'nome', t.nome, 'disciplina', dt.sigla, 'periodo', dt.periodo::int
        ))
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      'tema', jsonb_build_object(
                'id', sub.tema_id, 'nome', sub.tema_nome,
                'disciplina', sub.tema_disciplina, 'periodo', sub.tema_periodo,
                'parent_id', null, 'criado_em', null
              ),
      'total', sub.total,
      'acertos', sub.acertos
    )
  ...
  FROM (
    SELECT
      ...
      dt.sigla AS tema_disciplina, dt.periodo::int AS tema_periodo,
    ...
    LEFT JOIN disciplina dt ON dt.id = t.disciplina_id
    WHERE tr.tentativa_id = p_tentativa_id
    GROUP BY t.id, t.nome, dt.sigla, dt.periodo
  ) sub;
...
-- ============================================================
-- 9. Recriar listar_temas_com_contagem
-- ============================================================
