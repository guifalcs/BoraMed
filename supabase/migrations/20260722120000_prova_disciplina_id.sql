-- Vincula prova a disciplina (select no admin). Integradoras ficam sem
-- vínculo obrigatório (não pertencem a uma única matéria).

ALTER TABLE public.prova
  ADD COLUMN disciplina_id uuid REFERENCES public.disciplina(id) ON DELETE SET NULL;

CREATE INDEX idx_prova_disciplina_id ON public.prova (disciplina_id);

-- admin_criar_prova_com_questoes: gravar disciplina_id da prova.
-- Corpo idêntico ao de 20260715120000, apenas com o campo novo no INSERT.
...
BEGIN
  ...
  IF nullif(p_prova ->> 'disciplina_id', '') IS NOT NULL THEN
...
-- ─────────────────────────────────────────────────────────────────
-- Backfill: provas do período 1 já têm a matéria no nome (ex: "N1 SOI
-- 2025.1", "N1 HAM I 2024.2"). Integradoras não pertencem a uma matéria
-- específica e ficam de fora. Demais períodos: sem matérias cadastradas
-- ainda, backfill fica para depois.
-- ─────────────────────────────────────────────────────────────────
...
WHERE p.periodo = 1
  ...
  AND d.periodo = 1
  AND (
    (d.sigla = 'HAM I' AND p.nome ~* '\yHAM\y') OR
    (d.sigla = 'IESC I' AND p.nome ~* '\yIESC\y') OR
    (d.sigla = 'MCM I' AND p.nome ~* '\yMCM\y') OR
    (d.sigla = 'SOI I' AND p.nome ~* '\ySOI\y')
  );
