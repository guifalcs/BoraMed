...
BEGIN
  ...
  -- Marca correta em tentativa_resposta
  ...
  -- Atualiza estatísticas de questao
  UPDATE questao q
  ...
  -- Distribuição por tema — usa subquery para evitar aggregate aninhado
  SELECT jsonb_agg(
    jsonb_build_object(
      'tema_id',   sub.tema_id,
      'tema_nome', sub.tema_nome,
      'total',     sub.total,
      'acertos',   sub.acertos
    )
  )
...
BEGIN
  ...
  -- Valida que a prova existe e tem questões ativas
  SELECT p.*, COUNT(q.id) AS total
  INTO v_prova
  FROM prova p
  LEFT JOIN questao q ON q.prova_id = p.id AND q.status = 'ativa'
  WHERE p.id = p_prova_id
  GROUP BY p.id;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Prova não encontrada' USING ERRCODE = 'P0003';
  END IF;

  IF v_prova.total = 0 THEN
    RAISE EXCEPTION 'A prova não possui questões ativas' USING ERRCODE = 'P0004';
  END IF;
  ...
  VALUES (
    v_user_id, p_prova_id, p_modo, 'em_andamento',
    v_prova.total, 0, 0,
    NOW(), NOW()
  )
  RETURNING * INTO v_tentativa;

  -- Cria registros vazios de resposta para cada questão
  INSERT INTO tentativa_resposta (tentativa_id, questao_id)
  SELECT v_tentativa.id, q.id
  FROM questao q
  WHERE q.prova_id = p_prova_id AND q.status = 'ativa'
  ORDER BY q.ordem_na_prova;

  -- Monta as questões com alternativas (mascara correta se modo='simulado')
  SELECT jsonb_agg(
    jsonb_build_object(
      'id',                      q.id,
      'prova_id',                q.prova_id,
...
BEGIN
  ...
  -- Valida ownership e busca tentativa
  SELECT * INTO v_tentativa
  FROM tentativa
  WHERE id = p_tentativa_id AND user_id = v_user_id;
  ...
  -- Atualiza para em_andamento, zera pausada_em
  UPDATE tentativa
  ...
  -- Monta questões (mesma lógica de correta mascarada)
  SELECT jsonb_agg(
    jsonb_build_object(
      'id',                      q.id,
      'prova_id',                q.prova_id,
...
-- ==============================================================
-- SEEDS DE DEMONSTRAÇÃO — módulo provas
-- UUIDs fixos + ON CONFLICT DO NOTHING para idempotência
-- ==============================================================

-- Faculdade
INSERT INTO public.faculdade (id, nome, sigla, rede, ativa)
VALUES ('aaaaaaaa-0000-0000-0000-000000000001', 'Afya Faculdades', 'AFYA', 'Afya', true)
...
INSERT INTO public.prova (id, faculdade_id, nome, periodo, ano, semestre, tipo, subtipo_nacional, qtd_questoes, tempo_sugerido_minutos) VALUES
  ('cccccccc-0000-0000-0000-000000000001', 'aaaaaaaa-0000-0000-0000-000000000001',
   'Prova N1 2024 — Período 1', 1, 2024, 1, 'nacional', 'N1', 5, 60),
  ('cccccccc-0000-0000-0000-000000000002', 'aaaaaaaa-0000-0000-0000-000000000001',
   'Prova N2 2024 — Período 1', 1, 2024, 1, 'nacional', 'N2', 5, 60),
  ('cccccccc-0000-0000-0000-000000000003', 'aaaaaaaa-0000-0000-0000-000000000001',
   'Prova N1 2023 — Período 1', 1, 2023, 1, 'nacional', 'N1', 5, 60)
ON CONFLICT (id) DO NOTHING;

-- 5 questões para a Prova N1 2024
INSERT INTO public.questao (id, prova_id, ordem_na_prova, enunciado, formato, status, disciplina, periodo, dificuldade, revisado) VALUES
  ('dddddddd-0000-0000-0000-000000000001', 'cccccccc-0000-0000-0000-000000000001', 1,
   'O osso mais longo do corpo humano é:', 'multipla_escolha', 'ativa', 'Ciências Básicas', 1, 2, true),
  ('dddddddd-0000-0000-0000-000000000002', 'cccccccc-0000-0000-0000-000000000001', 2,
   'Qual é a principal função do coração?', 'multipla_escolha', 'ativa', 'Ciências Básicas', 1, 2, true),
  ('dddddddd-0000-0000-0000-000000000003', 'cccccccc-0000-0000-0000-000000000001', 3,
   'O processo de quebra de glicose para obtenção de energia é chamado de:', 'multipla_escolha', 'ativa', 'Ciências Básicas', 1, 3, true),
  ('dddddddd-0000-0000-0000-000000000004', 'cccccccc-0000-0000-0000-000000000001', 4,
   'Qual é a molécula responsável pelo transporte de oxigênio no sangue?', 'multipla_escolha', 'ativa', 'Ciências Básicas', 1, 2, true),
  ('dddddddd-0000-0000-0000-000000000005', 'cccccccc-0000-0000-0000-000000000001', 5,
   'O processo de síntese de proteínas a partir do RNA mensageiro é chamado de:', 'multipla_escolha', 'ativa', 'Ciências Básicas', 1, 3, true)
ON CONFLICT (id) DO NOTHING;

-- Alternativas — Questão 1 (correta: A = Fêmur)
...
-- Alternativas — Questão 2 (correta: C = Bombear sangue)
INSERT INTO public.alternativa (id, questao_id, letra, texto, correta, ordem) VALUES
...
-- Alternativas — Questão 3 (correta: B = Glicólise)
INSERT INTO public.alternativa (id, questao_id, letra, texto, correta, ordem) VALUES
...
-- Alternativas — Questão 4 (correta: D = Hemoglobina)
INSERT INTO public.alternativa (id, questao_id, letra, texto, correta, ordem) VALUES
...
-- Alternativas — Questão 5 (correta: E = Tradução)
INSERT INTO public.alternativa (id, questao_id, letra, texto, correta, ordem) VALUES
...
-- questao_tema: Q1 e Q4 = Anatomia, Q2 = Fisiologia, Q3 e Q5 = Bioquímica
