...
BEGIN
  ...
  -- Seleciona questões (EXISTS não multiplica linhas, DISTINCT desnecessário)
  SELECT ARRAY(
    SELECT q.id
    FROM questao q
  ...
  -- Gera edicao unica para evitar violar unique(tipo, periodo, edicao)
  -- Usa negativo do epoch em segundos para nunca colidir com edicoes normais (positivas)
