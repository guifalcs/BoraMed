...
begin
  ...
  return (
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'xp_total',xp_total,'xp_semana_atual',xp_semana_atual,'posicao',posicao) order by posicao),'[]'::jsonb)
...
begin
  ...
  return (
    with global_rank as (
      select user_id, row_number() over (order by xp_total desc, atualizado_em asc, user_id asc) as posicao
      from public.user_gamificacao_stats where xp_total > 0
    ),
    semana_rank as (
      select user_id, row_number() over (order by xp_semana_atual desc, atualizado_em asc, user_id asc) as posicao
      from public.user_gamificacao_stats where xp_semana_atual > 0
    ),
    totais as (
      select count(*) filter (where xp_total > 0)::integer as total_global,
    ...
    select jsonb_build_object(
      ...
      'total_global', total_global, 'total_semana', total_semana
    ) from totais
  );
end;
$function$;
