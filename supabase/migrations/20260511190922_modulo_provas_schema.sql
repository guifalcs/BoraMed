...
create policy "alternativa_select_authenticated" on "public"."alternativa" as permissive for select to authenticated using (true);
create policy "faculdade_select_authenticated" on "public"."faculdade" as permissive for select to authenticated using (true);
create policy "prova_select_authenticated" on "public"."prova" as permissive for select to authenticated using (true);
create policy "questao_select_authenticated" on "public"."questao" as permissive for select to authenticated using (true);
create policy "questao_tema_select_authenticated" on "public"."questao_tema" as permissive for select to authenticated using (true);
create policy "tema_select_authenticated" on "public"."tema" as permissive for select to authenticated using (true);

create policy "tentativa_insert_own" on "public"."tentativa" as permissive for insert to authenticated with check ((auth.uid() = user_id));
create policy "tentativa_select_own" on "public"."tentativa" as permissive for select to authenticated using ((auth.uid() = user_id));
create policy "tentativa_update_own" on "public"."tentativa" as permissive for update to authenticated using ((auth.uid() = user_id));

create policy "tentativa_resposta_insert_own" on "public"."tentativa_resposta" as permissive for insert to authenticated with check ((EXISTS ( SELECT 1 FROM public.tentativa t WHERE ((t.id = tentativa_resposta.tentativa_id) AND (t.user_id = auth.uid())))));
create policy "tentativa_resposta_select_own" on "public"."tentativa_resposta" as permissive for select to authenticated using ((EXISTS ( SELECT 1 FROM public.tentativa t WHERE ((t.id = tentativa_resposta.tentativa_id) AND (t.user_id = auth.uid())))));
create policy "tentativa_resposta_update_own" on "public"."tentativa_resposta" as permissive for update to authenticated using ((EXISTS ( SELECT 1 FROM public.tentativa t WHERE ((t.id = tentativa_resposta.tentativa_id) AND (t.user_id = auth.uid())))));
