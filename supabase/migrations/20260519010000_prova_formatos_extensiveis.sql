-- Modelo extensivel para formatos de prova.
-- Mantem prova.tipo como legado de origem simplificada enquanto o app migra
-- para origem/formato/rede/subtipo.

alter table public.prova
  add column if not exists origem text not null default 'autoral',
...
alter table public.prova
  drop constraint if exists prova_origem_check,
  drop constraint if exists prova_formato_check,
  drop constraint if exists prova_tipo_check;

alter table public.prova
  add constraint prova_origem_check
    check (origem in ('autoral', 'faculdade', 'personalizado')),
  add constraint prova_formato_check
    check (formato is null or formato in ('nacional', 'processual', 'laboratorio', 'multiestacoes'));

update public.prova
set
  origem = case
    when periodo = 0 or edicao < 0 then 'personalizado'
    when tipo = 'faculdade' then 'faculdade'
    else 'autoral'
  end,
  formato = coalesce(
    formato,
    case
      when tipo = 'nacional' or subtipo_nacional is not null then 'nacional'
      when tipo = 'processual' then 'processual'
      when tipo = 'multiestacoes' then 'multiestacoes'
      else null
    end
  ),
  subtipo = coalesce(subtipo, subtipo_nacional),
  rede = coalesce(rede, case when subtipo_nacional is not null then 'afya' else null end),
  publicada = case when periodo > 0 and qtd_questoes > 0 then true else publicada end,
  tipo = case when tipo = 'faculdade' then 'faculdade' else 'autoral' end;

alter table public.prova
  add constraint prova_tipo_check
    check (tipo in ('autoral', 'faculdade'));

create index if not exists idx_prova_formato on public.prova (formato);
create index if not exists idx_prova_origem on public.prova (origem);
create index if not exists idx_prova_rede on public.prova (rede);
create index if not exists idx_prova_publicacao on public.prova (publicada, arquivada);
