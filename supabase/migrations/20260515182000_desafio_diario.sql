...
-- 3. Tabela desafio_diario_resposta
...
create policy "desafio_diario_select"
  on "public"."desafio_diario"
  as permissive for select to authenticated
  using (true);

create policy "desafio_diario_resposta_select_own"
  on "public"."desafio_diario_resposta"
  as permissive for select to authenticated
...
begin
  ...
  -- Auto-pick: sorteia questão elegível se ainda não há desafio hoje
  ...
  -- Busca resposta do usuário (se já respondeu)
  ...
  if found and v_resposta.user_id is not null then
    -- Usuário já respondeu: retorna com correta e minha_resposta
    return jsonb_build_object(
      'disponivel',    true,
      'data',          v_hoje,
      'questao', (
    ...
    -- Usuário ainda não respondeu: alternativas SEM correta
    return jsonb_build_object(
      'disponivel',    true,
      'data',          v_hoje,
      'questao', (
...
begin
  ...
  -- Verifica se o desafio existe hoje
  select questao_id into v_questao_id
  from public.desafio_diario
  where data = v_hoje;
  ...
  -- Idempotência: se já respondeu, retorna resultado atual
  if exists (
  ...
  -- Valida alternativa pertence à questão do dia
  select * into v_alt
  ...
  -- Concede XP (idempotente via unique constraint)
