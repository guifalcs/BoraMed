...
begin
  ...
  insert into public.user_gamificacao_stats (
    user_id, xp_total, xp_semana_atual, semana_iso, nivel, ultimo_dia_ativo, atualizado_em
  )
  values (
    new.user_id, new.xp, new.xp, v_semana_iso,
    floor(sqrt(new.xp::numeric / 100))::smallint,
    (new.criado_em at time zone 'America/Sao_Paulo')::date, now()
  )
...
begin
  ...
  if found then
    return jsonb_build_object('xp_ganho', 0, 'ja_concedido', true, 'novas_conquistas', '[]'::jsonb, 'stats', public.get_meu_xp());
  end if;
  ...
  v_bonus_tempo := case
    when v_tempo_medio is not null and v_tempo_medio < 60 and coalesce(v_tentativa.nota, 0) >= 50 then 15
    else 0
  end;
  ...
  values (
    ...
    jsonb_build_object(
      'tentativa_id', p_tentativa_id, 'xp_calculado', v_xp_calculado,
      'base', v_base, 'bonus_nota', v_bonus_nota,
      'bonus_dificuldade', v_bonus_dificuldade, 'bonus_tempo', v_bonus_tempo
    ),
    v_idempotency_key
  );
  ...
  return jsonb_build_object('xp_ganho', v_xp_concedido, 'ja_concedido', false, 'novas_conquistas', '[]'::jsonb, 'stats', v_stats);
end;
$function$;
