-- Adiciona parâmetro p_tempo_segundos à finalizar_tentativa para persistir
-- o tempo real medido pelo timer do frontend no campo tempo_acumulado_segundos.
...
BEGIN
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'alternativas', (
        SELECT jsonb_agg(jsonb_build_object('id', a.id, 'questao_id', a.questao_id, 'letra', a.letra, 'texto', a.texto, 'correta', a.correta, 'ordem', a.ordem, 'imagem_url', a.imagem_url) ORDER BY a.ordem)
        FROM alternativa a WHERE a.questao_id = q.id
      ),
      'temas', (
        SELECT jsonb_agg(jsonb_build_object('id', t.id, 'nome', t.nome, 'disciplina', t.disciplina, 'periodo', t.periodo))
        FROM questao_tema qt JOIN tema t ON t.id = qt.tema_id WHERE qt.questao_id = q.id
      )
  ...
  FROM (
    SELECT
      ...
      t.disciplina AS tema_disciplina, t.periodo AS tema_periodo,
