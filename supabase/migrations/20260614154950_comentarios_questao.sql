...
create index idx_questao_comentario_questao_id on public.questao_comentario (questao_id);
create index idx_questao_comentario_parent_id on public.questao_comentario (parent_id);
create index idx_questao_comentario_user_id on public.questao_comentario (user_id);
create index idx_questao_comentario_questao_status_criado on public.questao_comentario (questao_id, status, criado_em);
...
begin
  if new.parent_id is not null then
    if exists (select 1 from public.questao_comentario p where p.id = new.parent_id and p.parent_id is not null) then
      raise exception 'Respostas nao podem ter sub-respostas' using errcode = 'P0002';
    end if;
    if not exists (select 1 from public.questao_comentario p where p.id = new.parent_id and p.questao_id = new.questao_id) then
...
create policy questao_comentario_select on public.questao_comentario for select to authenticated
  using (status = 'ativo' or user_id = (select auth.uid()) or public.is_admin());
create policy questao_comentario_insert on public.questao_comentario for insert to authenticated
  with check (user_id = (select auth.uid()));
create policy questao_comentario_update on public.questao_comentario for update to authenticated
  using (user_id = (select auth.uid())) with check (user_id = (select auth.uid()));
create policy questao_comentario_delete on public.questao_comentario for delete to authenticated
...
create index idx_questao_comentario_voto_comentario on public.questao_comentario_voto (comentario_id);
create index idx_questao_comentario_voto_user on public.questao_comentario_voto (user_id);

create or replace function public.trg_fn_comentario_voto_recalcular()
returns trigger language plpgsql security definer set search_path to 'public', 'pg_temp'
as $$
declare v_comentario_id uuid;
begin
  v_comentario_id := coalesce(new.comentario_id, old.comentario_id);
  update public.questao_comentario set
    likes = (select count(*) from public.questao_comentario_voto where comentario_id = v_comentario_id and valor = 1),
    dislikes = (select count(*) from public.questao_comentario_voto where comentario_id = v_comentario_id and valor = -1)
...
create policy questao_comentario_voto_select on public.questao_comentario_voto for select to authenticated using (user_id = (select auth.uid()));
create policy questao_comentario_voto_insert on public.questao_comentario_voto for insert to authenticated with check (user_id = (select auth.uid()));
create policy questao_comentario_voto_update on public.questao_comentario_voto for update to authenticated using (user_id = (select auth.uid())) with check (user_id = (select auth.uid()));
create policy questao_comentario_voto_delete on public.questao_comentario_voto for delete to authenticated using (user_id = (select auth.uid()));
...
create table public.questao_comentario_denuncia (
  ...
  motivo text,
  criado_em timestamptz not null default now(),
  constraint questao_comentario_denuncia_motivo_check check (motivo is null or char_length(btrim(motivo)) <= 500),
  constraint questao_comentario_denuncia_unique unique (comentario_id, user_id)
);

create index idx_questao_comentario_denuncia_comentario on public.questao_comentario_denuncia (comentario_id);
...
create policy questao_comentario_denuncia_insert on public.questao_comentario_denuncia for insert to authenticated with check (user_id = (select auth.uid()));
create policy questao_comentario_denuncia_select_admin on public.questao_comentario_denuncia for select to authenticated using (public.is_admin());
...
begin
  ...
  for v_termo in select termo from public.palavra_proibida loop
    ...
    if v_texto_norm ~ ('\m' || v_termo_norm || '\M') then return true; end if;
  end loop;
  return false;
end;
$$;
...
create or replace function public.listar_comentarios_questao(p_questao_id uuid, p_ordenacao text default 'relevante')
...
create or replace function public.criar_comentario_questao(p_questao_id uuid, p_conteudo text, p_parent_id uuid default null)
...
begin
  if v_user_id is null then raise exception 'Usuario nao autenticado' using errcode = 'P0001'; end if;
  if v_conteudo = '' then raise exception 'Conteudo nao pode ser vazio' using errcode = 'P0004'; end if;
  if char_length(v_conteudo) > 2000 then raise exception 'Conteudo muito longo' using errcode = 'P0005'; end if;
  if public.contem_palavra_proibida(v_conteudo) then raise exception 'Comentario contem linguagem inapropriada' using errcode = 'P0010'; end if;
  if p_parent_id is not null then
    if not exists (select 1 from public.questao_comentario p where p.id = p_parent_id and p.questao_id = p_questao_id and p.parent_id is null and p.status = 'ativo') then
      raise exception 'Comentario pai invalido' using errcode = 'P0006';
    end if;
  end if;
  insert into public.questao_comentario (questao_id, user_id, parent_id, conteudo)
  values (p_questao_id, v_user_id, p_parent_id, v_conteudo) returning * into v_novo;
  select * into v_profile from public.profiles where id = v_user_id;
  return jsonb_build_object(
    'id', v_novo.id, 'parent_id', v_novo.parent_id, 'conteudo', v_novo.conteudo,
    'status', v_novo.status, 'editado', v_novo.editado,
    'nome_display', case when coalesce(v_profile.competir_publico, true) then coalesce(nullif(v_profile.nome_completo, ''), split_part(v_profile.email, '@', 1), 'Aluno') else 'Anônimo' end,
    'avatar_url', case when coalesce(v_profile.competir_publico, true) then v_profile.avatar_url else null end,
    'user_id', v_user_id, 'is_me', true, 'likes', 0, 'dislikes', 0, 'meu_voto', 0,
    'criado_em', v_novo.criado_em, 'respostas', '[]'::jsonb
  );
end;
$$;
...
create or replace function public.editar_comentario_questao(p_comentario_id uuid, p_conteudo text)
returns jsonb language plpgsql security invoker set search_path to 'public', 'pg_temp'
as $$
declare
  v_user_id uuid := auth.uid();
...
begin
  ...
  if char_length(v_conteudo) > 2000 then raise exception 'Conteudo muito longo' using errcode = 'P0005'; end if;
  if public.contem_palavra_proibida(v_conteudo) then raise exception 'Comentario contem linguagem inapropriada' using errcode = 'P0010'; end if;
  select * into v_comentario from public.questao_comentario where id = p_comentario_id and user_id = v_user_id and status = 'ativo';
  if not found then raise exception 'Comentario nao encontrado ou sem permissao' using errcode = 'P0007'; end if;
  update public.questao_comentario set conteudo = v_conteudo, editado = true where id = p_comentario_id returning * into v_comentario;
  select * into v_profile from public.profiles where id = v_user_id;
  return jsonb_build_object(
    'id', v_comentario.id, 'parent_id', v_comentario.parent_id, 'conteudo', v_comentario.conteudo,
    'status', v_comentario.status, 'editado', v_comentario.editado,
    'nome_display', case when coalesce(v_profile.competir_publico, true) then coalesce(nullif(v_profile.nome_completo, ''), split_part(v_profile.email, '@', 1), 'Aluno') else 'Anônimo' end,
    'avatar_url', case when coalesce(v_profile.competir_publico, true) then v_profile.avatar_url else null end,
    'user_id', v_user_id, 'is_me', true,
    'likes', v_comentario.likes, 'dislikes', v_comentario.dislikes,
    'meu_voto', coalesce((select valor from public.questao_comentario_voto where comentario_id = p_comentario_id and user_id = v_user_id), 0),
    'criado_em', v_comentario.criado_em, 'respostas', '[]'::jsonb
  );
end;
$$;
...
begin
  if v_user_id is null then raise exception 'Usuario nao autenticado' using errcode = 'P0001'; end if;
  if not exists (select 1 from public.questao_comentario where id = p_comentario_id and (user_id = v_user_id or public.is_admin())) then
    raise exception 'Comentario nao encontrado ou sem permissao' using errcode = 'P0007';
  end if;
  select exists (select 1 from public.questao_comentario where parent_id = p_comentario_id and status = 'ativo') into v_tem_respostas;
  if v_tem_respostas then
    update public.questao_comentario set status = 'removido' where id = p_comentario_id;
  else
...
create or replace function public.votar_comentario_questao(p_comentario_id uuid, p_valor smallint)
returns jsonb language plpgsql security invoker set search_path to 'public', 'pg_temp'
as $$
declare
  v_user_id uuid := auth.uid();
...
begin
  if v_user_id is null then raise exception 'Usuario nao autenticado' using errcode = 'P0001'; end if;
  if p_valor not in (-1, 1) then raise exception 'Valor de voto invalido' using errcode = 'P0008'; end if;
  select valor into v_voto_atual from public.questao_comentario_voto where comentario_id = p_comentario_id and user_id = v_user_id;
  if v_voto_atual = p_valor then
    delete from public.questao_comentario_voto where comentario_id = p_comentario_id and user_id = v_user_id;
  else
    insert into public.questao_comentario_voto (comentario_id, user_id, valor) values (p_comentario_id, v_user_id, p_valor)
...
create or replace function public.denunciar_comentario_questao(p_comentario_id uuid, p_motivo text default null)
...
as $$
  ...
  values (p_comentario_id, auth.uid(), case when btrim(coalesce(p_motivo, '')) = '' then null else btrim(p_motivo) end)
