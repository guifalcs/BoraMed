-- Modo "todos" no montar-simulado deve incluir apenas questões processuais e de
-- laboratório, excluindo nacional. Anteriormente p_tipo_questao = NULL retornava
-- todos os tipos (incluindo 'nacional'). Agora NULL significa "sem filtro de tipo
-- específico, mas excluindo nacional". Para incluir nacional, passe p_tipo_questao = 'nacional'.
...
AS $function$
  ...
  LEFT JOIN questao q
    ...
    AND q.status = 'ativa'
    AND (
      (p_tipo_questao IS NULL AND q.tipo_questao <> 'nacional')
      OR q.tipo_questao = p_tipo_questao
    )
