-- admin_criar_prova_com_questoes passa a aceitar questões discursivas
-- (formato 'resposta_aberta_curta'): valida resposta_modelo em vez de
-- alternativas/gabarito e grava resposta_modelo/pontos_chave/criterios_correcao.
-- Fechadas mantêm a validação original (≥2 alternativas, gabarito único).
-- Também troca RETURNING * por RETURNING id no INSERT de questao: as colunas
-- de gabarito aberto são secretas (sem SELECT grant) e o RETURNING * quebraria
-- a função (SECURITY INVOKER) para qualquer inserção.
