-- Segurança MÉDIO — ranking não deve desanonimizar perfil privado.
-- Antes: get_ranking_global/_semana punham nome_display='Anônimo' quando
-- competir_publico=false, mas devolviam o user_id real + XP exato, permitindo
-- reidentificar e enumerar user_ids. Agora o user_id é mascarado (NULL) quando
-- o perfil é privado e não é o próprio usuário (is_me).
...
begin
  ...
  return (
    with ranking as (
      select
        s.user_id,
        coalesce(p.competir_publico, s.competir_publico, true) as publico,
        case
    ...
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          'user_id',        case when publico or is_me then user_id else null end,
