    ...
    if (type === "subscription_preapproval") {
      ...
      if (sub) {
        ...
        if (userId) {
          ...
          // Só sobrescreve proxima_cobranca com data FUTURA e em assinatura
          // ATIVA. O preapproval nasce com next_payment_date = date_created (a
          // 1ª fatura processa assíncrono); gravar essa data ≤ agora apagaria o
          // acesso provisório concedido pelo mp-processar-assinatura e trancaria
          // o assinante no paywall até a 1ª cobrança real (bug visto em produção
          // em 2026-07-09). Em CANCELAMENTO, o next_payment_date vira a data do
          // retry abortado (1 mês à frente) — gravá-la estenderia a carência de
          // quem nunca pagou (visto em produção em 2026-07-10, recusa da 1ª
          // cobrança); a carência correta é a que já está no banco.
          if (
            status === "authorized" &&
            nextPayment &&
            new Date(nextPayment).getTime() > deps.now().getTime()
          ) {
      ...
      if (ap) {
        ...
        const apPay = ap["payment"] as
          ...
          | undefined;
        if (status !== "approved" && apPay?.status === "rejected") {
          status = "rejected";
        }
        ...
        const apTd = ap["transaction_details"] as {
          net_received_amount?: number;
        } | undefined;
        let liquidoCentavos = apTd?.net_received_amount != null
          ? Math.round(apTd.net_received_amount * 100)
          : null;
        ...
        if (apPay?.id != null) {
          ...
          if (realPay) {
            ...
            metodo = (realPay["payment_method_id"] as string | undefined) ??
              null;
          }
        }
