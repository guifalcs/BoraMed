...
export async function handleProcessarAssinatura(
  req: Request,
  deps: Deps,
): Promise<Response> {
  ...
  // Sem ele, a 1ª cobrança recorrente vinha sendo recusada com
  // cc_rejected_high_risk (visto em produção em 2026-07-10).
  const deviceId = typeof body.device_id === "string" &&
      /^[\w.:-]{1,256}$/.test(body.device_id)
    ? body.device_id
    : undefined;
  ...
  if (
    hasActiveAccess(
      linhasAcesso.filter((a) => a.status !== "paused"),
      deps.now().getTime(),
    )
  ) {
  ...
  if (linhasAcesso.some((a) => a.status === "paused")) {
    return reply(
      {
        error:
          'Sua assinatura está pausada. Reative-a em "Minha assinatura" para voltar a estudar.',
      },
      409,
    );
  }
  ...
  if (!existente) {
    const desde = new Date(deps.now().getTime() - RATE_LIMIT_WINDOW_MS)
      .toISOString();
  ...
  if (!res.ok) {
    const detail = (res.body["message"] as string | undefined) ??
    ...
    if (res.status >= 500) {
      await admin
        ...
        .eq("id", intencaoId);
      return reply({
        error: "Pagamento temporariamente indisponível. Tente novamente.",
      }, 502);
    }
  ...
  // MP cancelar, a carência do provisório expira em ate 72h em vez de 1 mês (cobre retries de cobrança do MP) —
  ...
  await admin.from("assinatura").upsert(
    {
      ...
      status,
      data_inicio: (pre["date_created"] as string | undefined) ??
