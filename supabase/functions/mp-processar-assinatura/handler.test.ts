...
Deno.test('processar-assinatura authorized com next_payment_date = agora → acesso provisório (+1 dia)', async () => {
  const db = baseDb();
  ...
  assertExists(assin);
  // NOW = 2026-06-24 → concede +1 dia provisório (a 1ª cobrança real grava a
  // data verdadeira via webhook; se falhar, o acesso expira em 24h).
  assertEquals(assin?.proxima_cobranca, '2026-06-25T12:00:00.000Z');
});
