...
function request(body: unknown, auth = "Bearer jwt"): Request {
  return new Request(
    "https://proj.supabase.co/functions/v1/mp-processar-assinatura",
    {
      method: "POST",
      headers: {
...
Deno.test("processar-assinatura: attempt_id inválido / sem card_token → 400", async () => {
  ...
  assertEquals(
    (await handleProcessarAssinatura(
      request(goodBody({ card_token_id: undefined })),
      deps(),
    )).status,
    400,
  );
...
Deno.test("processar-assinatura: assinatura pausada → 409 direciona p/ reativar, sem 2º preapproval", async () => {
  ...
  assertEquals(
    calls.length,
    0,
    "não cria um 2º preapproval para quem está pausado",
  );
...
Deno.test("processar-assinatura authorized: payload correto (preço do banco, status authorized, e-mail da conta), assinatura criada", async () => {
  ...
  assertEquals(
    db.rows("pagamento").length,
    0,
    "cobrança de verificação não vira pagamento",
  );
});
...
Deno.test("processar-assinatura authorized supera assinatura anterior (B5)", async () => {
  const db = baseDb({
    assinatura: [
      ...
      // caso real do semestral vencido.
      {
        ...
        proxima_cobranca: "2026-01-01T00:00:00.000Z",
      },
    ],
  });
  ...
  assertEquals(res.status, 200);
  assertEquals(
    find(db, "assinatura", (r) => r.id === "old")?.status,
    "cancelled",
  );
  assertEquals(
    find(db, "assinatura", (r) => r.mp_preapproval_id === "PRE-1")?.status,
    "authorized",
  );
});
...
Deno.test("processar-assinatura: cartão recusado (4xx do MP) → 200 rejected + intenção recusada, sem assinatura", async () => {
  ...
  const { fn } = captureFetch({
    status: 400,
    body: { message: "Cannot process card" },
  });
  const res = await handleProcessarAssinatura(
    request(goodBody()),
    makeDeps({
...
Deno.test("processar-assinatura: device_id vai no header X-meli-session-id (whitelisted)", async () => {
  // Válido → repassado no POST /preapproval (antifraude do MP).
  const ok = captureFetch({ body: authorizedPre });
  await handleProcessarAssinatura(
    request(goodBody({ device_id: "armor.abc123-DEF_456:seg" })),
    makeDeps({
      db: baseDb(),
      fetch: ok.fn,
      now: NOW,
  ...
  const bad = captureFetch({ body: { ...authorizedPre, id: "PRE-2" } });
  await handleProcessarAssinatura(
    request(
      goodBody({
        attempt_id: "aaaaaaaa-bbbb-cccc-dddd-ffffffffffff",
        device_id: 'x"; DROP TABLE --',
      }),
    ),
    makeDeps({
      db: baseDb(),
