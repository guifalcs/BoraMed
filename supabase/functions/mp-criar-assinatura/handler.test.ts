...
function req(body: unknown, withAuth = true): Request {
  ...
  return new Request(
    "https://proj.supabase.co/functions/v1/mp-criar-assinatura",
    {
      method: "POST",
      headers,
...
Deno.test("criar-assinatura: plano_slug ausente → 400", async () => {
  const res = await handleCriarAssinatura(
    req({}),
    makeDeps({ caller: { id: "u1", email: "a@b.com" } }),
  );
...
Deno.test("criar-assinatura recorrente: cria preapproval (external_reference persistido) e assinatura pending", async () => {
  ...
  const fetch = fakeFetch([
    {
      ...
      body: {
        id: "PRE-NEW",
        init_point: "https://mp/sub/PRE-NEW",
  ...
  const res = await handleCriarAssinatura(
    ...
    makeDeps({
      db,
      caller: { id: "user-abc", email: "a@b.com" },
      fetch,
      now: NOW,
    }),
  );
  ...
  const assin = db.rows("assinatura").find((r) =>
    r.mp_preapproval_id === "PRE-NEW"
  );
...
Deno.test("criar-assinatura único (semestral): cria preferência e devolve init_point", async () => {
  ...
  const fetch = fakeFetch([
    {
      match: "/checkout/preferences",
      body: { init_point: "https://mp/pref-xyz" },
    },
  ]);
  const res = await handleCriarAssinatura(
