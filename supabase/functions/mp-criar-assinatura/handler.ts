...
export async function handleCriarAssinatura(
  req: Request,
  deps: Deps,
): Promise<Response> {
  ...
  const { data: plano, error: planoError } = await admin
    .from("plano")
    .select(
      "id, nome, slug, mp_init_point, ativo, recorrente, preco_centavos, frequency, frequency_type, moeda",
    )
    .eq("slug", planoSlug)
    .single();
  if (planoError || !plano) {
    return reply({ error: "plano não encontrado" }, 404);
  }
  ...
  const prefRes = await deps.fetch(
  ...
  if (!initPoint) {
    return reply({ error: "checkout sem init_point", detail: pref }, 502);
  }
